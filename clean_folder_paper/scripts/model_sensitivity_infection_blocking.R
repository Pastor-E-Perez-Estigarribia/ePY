...
sirv_model <- odin::odin({
  ...
  # 
  ndoses = vaccination * population * 0.7585185 #Multiply by prop of pop over 12yo
  s_to_i <- scaling * state_s * state_i / population

  s_to_sv <- ve * ndoses * state_s/(state_s+state_r)
  r_to_rv <- ve * ndoses * state_r/(state_s+state_r)
  
  i_to_r = sir_gamma * state_i + introduction_i
  
  state_r = population-state_s-state_i-state_sv-state_rv
  deriv(state_s) <- -s_to_i - s_to_sv # Susceptible
  deriv(state_sv) = s_to_sv
  deriv(state_rv) = r_to_rv
  deriv(state_i) <-  s_to_i - i_to_r # Infectious
  ...
  output(stat_r) = state_r
  output(i_inf) <- s_to_i # Incident number of infections
  output(dose) = ndoses
  output(vaccinated) = state_sv+state_rv
  ...
  interp_vaccination[] = user()
  
  dim(interp_ts) <- user()
  dim(interp_vaccination) = length(interp_ts)
...
################################################################################
create_model_vac <- function(data, population, fits_coeff, location, prop_immune0, sir_gamma,
                         temperature_function, rainfall_function,
                         scaling_function, introduction_function,
                         vaccine_function, cov, ve, delay) {
  ...
  # vaccine_function = reactive_baseline
  ...
  ##############################################################################
  # Vaccination campaign (for infection blocking)
  ##############################################################################
  interp_vaccination = function(cov) vaccine_function(t_date = ts, coverage=cov, delay=delay)
  ...
  simulator <- sirv_model$new(
    ...
    interp_vaccination = interp_vaccination(cov),
    ve=ve
  )
