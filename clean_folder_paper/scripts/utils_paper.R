...
################################################################################
# Function to create vaccination campaign (only for reactive campaign using sensitivity analysis)
################################################################################
reactive_baseline = function(t_date, coverage, delay){
  t_date = as.Date(t_date)
  vr_vect = rep(0, length(t_date))
  window_campaign = t_date>"2022-10-03" & t_date<(as.Date("2022-10-03")+(3*30+2))
  vr_vect[window_campaign] = coverage/sum(window_campaign)
  #Add delay
  vr_vect = c(rep(0,100),vr_vect,rep(0,10))[(101:(length(t_date)+100))-delay]
  return(vr_vect)
}
