...
write.csv(nominal_df , "data/processed_data/nominal_data_2024012430.csv", row.names = F)
...
#  filter(date >= as.Date('2022-10-01')) %>% 
  ...
  mutate(name=factor(name,
                     ...
                     labels=c(
                              ...
                              "Metropolitano",
                              "Paraguay"))) %>% 
  dplyr::select(name,date, incidence) %>% 
...
ggsave(
  "output/fig/p_heatmap_inc_Figure_S1.pdf",
  plot = p_heatmap_inc,
  width = 22.65248,
  height = 14,
  units = "cm",
...
ggsave(
  "output/fig/p_heatmap_inc_Figure_S1.png",
  plot = p_heatmap_inc,
  width = 22.65248,
  height = 14,
  units = "cm",
...
ggsave(
  "output/fig/p_heatmap_inc_Figure_S1.svg",
  plot = p_heatmap_inc,
  width = 22.65248,
  height = 14,
  units = "cm",
...
sero_df_as = 
  ...
  dplyr::rename(Age_group = Var1,
    ...
    mutate(N = NO + YES,
           Fr = 100*YES/N)  %>% 
    dplyr::select(Age_group, Sex, YES, N, Fr) %>% 
    dplyr::rename(n_pos =YES, 
                  n_tot= N) %>% 
    mutate(date="2023-09-02") %>% 
    dplyr::select(date, Age_group, Sex, n_pos, n_tot, Fr)  %>% 
  mutate(Estimate = Hmisc::binconf(x = n_pos, n=n_tot,alpha=0.05,
                                   method="wilson")[,1]*100,
         CI_Lower = Hmisc::binconf(x = n_pos, n=n_tot,alpha=0.05,
                                   method="wilson")[,2]*100,
...
sero_df_as_a = 
  as.data.frame(table(sero_10$Age_group,sero$Seropositive)) %>% 
  pivot_wider(names_from = Var2, values_from = Freq) %>% 
  dplyr::rename(Age_group = Var1) %>% 
  mutate(N = NO + YES,
         Fr = 100*YES/N)  %>% 
  dplyr::select(Age_group, YES, N, Fr) %>% 
  dplyr::rename(n_pos =YES, 
                n_tot= N) %>% 
  mutate(date="2023-09-02") %>% 
  dplyr::select(date, Age_group,  n_pos, n_tot, Fr)  %>% 
  mutate(Estimate = Hmisc::binconf(x = n_pos, n=n_tot,alpha=0.05,
                                   method="wilson")[,1]*100,
         CI_Lower = Hmisc::binconf(x = n_pos, n=n_tot,alpha=0.05,
                                   method="wilson")[,2]*100,
...
p_load(AMR,DescTools)

## Formula interface for tabulated data plus shading and legend:
vcd::mosaic(~Seropositive + Age_group + Sex, data = sero, main = " ", 
       shade = TRUE, legend = T)

plot(table(sero$Seropositive,sero$Age_group))

GTest(table(sero$Seropositive,sero$Age_group))

plot(table(sero$Seropositive,sero$Sex))

GTest(table(sero$Seropositive,sero$Sex))
...
pop_df <- import("data/raw_data/Paraguay-2023b.csv", encoding = "Latin-1") %>%
...
unique(pop_df$age_group_by)

unique(nominal_df$age_group_10)
...
case_df_by_age_and_sex = nominal_df %>%
  ...
  full_join(.,sero_df_as %>% 
              dplyr::select(Sex,Age_group,Estimate,CI_Lower,CI_Upper), 
            by = c("Sex","Age_group")) %>% 
  ...
  mutate(Age_group=factor(Age_group,
                             ...
                             labels=c("<1",
                                      ...
                                      "60-69",
                                      ">70"))) %>% 
  mutate(Estimate_m_Upper = mean(sero_df_as$CI_Upper),
         CI_fm_Upper = projection*Estimate_m_Upper/100,
         CI_f_Upper = projection*CI_Upper/100,
         CI_0_Upper = projection*0.0185, #0.001,
         TI_m_Upper = CI_fm_Upper-CI_0_Upper,
         TI_Upper = CI_f_Upper-CI_0_Upper,
         IFR_Lower = 100*d_cases/TI_Upper,
         IFR_m_Lower= 100*d_cases/TI_m_Upper,
         pD_Lower = i_cases/TI_m_Upper) %>% 
  mutate(Estimate_m_Lower = mean(sero_df_as$CI_Lower),
         CI_fm_Lower = projection*Estimate_m_Lower/100,
         CI_f_Lower = projection*CI_Lower/100,
         CI_0_Lower = projection*0.0185, #0.001,
         TI_m_Lower = CI_fm_Lower-CI_0_Lower,
         TI_Lower = CI_f_Lower-CI_0_Lower,
         IFR_Upper = 100*d_cases/TI_Lower,
         IFR_m_Upper= 100*d_cases/TI_m_Lower,
         pD_Upper = i_cases/TI_m_Lower) %>% 
  dplyr::select(-CI_fm,-CI_f,-CI_0,
    -CI_fm_Upper,-CI_f_Upper,-CI_0_Upper,
           -CI_fm_Lower,-CI_f_Lower,-CI_0_Lower)
...
sero_df_as %>% 
  dplyr::select(Sex,Age_group,Estimate)
...
case_df_by_age_and_sex %>% 
  group_by(Sex) %>% 
  summarise(sum_c = sum(i_cases),
            sum_d = sum(d_cases)) %>% 
  mutate(CFR_S = 100*sum_d/sum_c)


A = matrix(c(81483, 122 , 60926, 176), nrow = 2)
...
b <- rbinom( prob=176/60926, size=1, n=60926)
ci.binom(b) # direct call
class(b) <- 'binom'
ci(b)       # indirect call

# binomial proportion and exact confidence interval
b <- rbinom(prob=122/81483, size=1, n=81483)
ci.binom(b) # direct call
class(b) <- 'binom'
ci(b)       # indirect call

df_CFR_a = case_df_by_age_and_sex %>% 
  group_by(Age_group) %>% 
  summarise(sum_c = sum(i_cases),
            sum_d = sum(d_cases)) %>% 
  mutate(CFR_a = 100*sum_d/sum_c) 

t.test(df_CFR_a$CFR_a)

# mean and confidence interval
ci(df_CFR_a$CFR_a)

summary(df_CFR_a$CFR_a)
...
# if (interactive()) {
#   ggraptR(case_df_by_age_and_sex)
# }

# IFR
sero_df_as_a$Age_group = as.character(sero_df_as_a$Age_group)

case_df_by_age = nominal_df %>%
  filter(classification!="DESCARTADO") %>%#Remove discarded cases
  filter(disease =="CHIKUNGUNYA") %>%
  filter(sex =="MASCULINO" | sex =="FEMENINO") %>%
  filter(date >= as.Date('2022-09-01') & date <= as.Date('2023-09-30')) %>% 
  ...
  full_join(., pop_df %>% subset(age_group_by == "age_group_10" &
                                   demographic_level == "COUNTRY"),
            by = c("age_group_10"="age_group")) %>% 
#  replace(is.na(.), 0) %>% 
  mutate(cfr = 100*d_cases/i_cases,
         incidence = 10000*i_cases/projection,
         age_specific_mortality = 10000*d_cases/projection) %>% 
  dplyr::rename(Age_group=age_group_10) %>% 
  full_join(.,sero_df_as_a %>% 
              dplyr::select(Age_group,Estimate,CI_Lower,CI_Upper), 
            by = c("Age_group")) %>% 
  mutate(Estimate_m = mean(sero_df_as_a$Estimate),
         CI_fm = projection*Estimate_m/100,
         CI_f = projection*Estimate/100,
         CI_0 = projection*0.0185, #0.001,
         TI_m = CI_fm-CI_0,
...
summary(case_df_by_age$cfr)

t.test(case_df_by_age$cfr)

summary(case_df_by_age$IFR_m)

t.test(case_df_by_age$IFR_m)


Model_cfr <- lm(IFR_m~ 0 + cfr,case_df_by_age_and_sex)
summary(Model_cfr)

gg3 = case_df_by_age_and_sex %>%  drop_na() %>% 
ggplot(aes(y = IFR_m, x = cfr)) + geom_point(
  aes(colour = Sex),
  stat = "identity",
  position = "identity",
  size = 2
  ...
  theme(legend.position = "none",
        ...
        text = element_text(
          ...
          hjust = 0.5,
          vjust = 0.5)) +
  ylab(expression(paste(IFR(bar(S))," %")))+
  xlab("CFR %") +
  labs(title = expression(paste(IFR(bar(S)) == 0.18*CFR + epsilon,
                                "; ",R^2== 0.98))) 
...
t.test(case_df_by_age_and_sex$cfr)
...
Range(case_df_by_age_and_sex$IFR_m)


# plotmath expression wrapped in expression()

Model <- lm(IFR~0 + offset(IFR_m),case_df_by_age_and_sex)
Residuals <- summary(Model)$residuals
SumResSquared <- sum(Residuals^2)
TotalSumSquares <- sum((case_df_by_age_and_sex$IFR - mean(case_df_by_age_and_sex$IFR, na.rm = T))^2, na.rm = T)
RSquared <- 1 - (SumResSquared/TotalSumSquares)
RSquared

gg2 = case_df_by_age_and_sex %>%
  drop_na() %>% 
  ggplot(aes(x=IFR_m, y=IFR,colour = Sex)) + 
  geom_point(size = 2) + 
  theme_bw() +
  geom_abline(slope = 1, 
              intercept = 0) + 
  theme(legend.position = "none",
        ...
        text = element_text(
          ...
          hjust = 0.5,
          vjust = 0.5)) +
  ylab(expression(paste(IFR(S), " %")))+
  xlab(expression(paste(IFR(bar(S)), " %"))) +
  labs(title = expression(paste(IFR(S) == IFR(bar(S)) + epsilon,
                                "; ",R^2== 0.95))) 

#expression(y == beta[0] + beta[1]*x + epsilon)

p_IFR = gg1 +
  inset_element(gg2, left = 0.05, bottom = 0.399, right = 0.4+.02, top = 1) +
  inset_element(gg3, left = 0.43+.02, bottom = 0.41, right = 0.429+0.35+.05, top = 1)
...
gg6 = df_sexIFR  %>%
  subset(Age_group != "100+") %>% 
  ggplot(aes(x=Age_group, y=IFRm_Ratio)) + 
  geom_segment(aes(x=Age_group, xend=Age_group, y=1, yend=IFRm_Ratio, color=mycolor), size=1.3, alpha=0.5) +
  geom_point(aes(color=mycolor), size = 3) + 
  theme_bw() +
  theme(
  ...
  geom_hline(yintercept=1, linetype="dashed", 
             color = "darkgray") +
  xlab(" ") +
  ylab(expression(IFR(bar(S))*Ratio == IFR(bar(S))["M"]*IFR(bar(S))["F"]^-1)) 
...
gg4 = case_df_by_age_and_sex %>%
  subset(Age_group != "100+") %>% 
  ggplot(aes(x=Age_group)) + 
  geom_bar(aes(y=pD,fill = Sex),
           stat = "identity",
           position = "dodge",
           alpha = 0.5)  +
  guides(fill=guide_legend(title=" ")) +
  ylab(expression(Pr["+"]==C[a]*I[a](bar(S))^-1)) +
  xlab("Age group")

gg4

names(case_df_by_age_and_sex)

df_sex = full_join(case_df_by_age_and_sex %>% 
                 subset(Sex != "M") %>% 
                 dplyr::select(Age_group, pD) %>% 
                   dplyr::rename(F= pD), 
                 case_df_by_age_and_sex %>% 
                 subset(Sex != "F") %>% 
                 dplyr::select(Age_group, pD) %>% 
  ...
  mutate(Pr_Ratio =F/M,
         mean_Pr_Ratio= mean(Pr_Ratio)) 


t.test(df_sex$Pr_Ratio, conf.level = 0.95)
...
gg5 = df_sex  %>%
  subset(Age_group != "100+") %>% 
ggplot(aes(x=Age_group, y=Pr_Ratio)) + 
  geom_segment(aes(x=Age_group, xend=Age_group, y=1, yend=Pr_Ratio, color=mycolor), size=1.3, alpha=0.5) +
  geom_point(aes(color=mycolor), size = 3) + 
  theme_bw() +
  theme(
    legend.position = "none",
  ...
  geom_hline(yintercept=1, linetype="dashed", 
               color = "darkgray") +
  xlab(" ") +
  ylab(expression(Pr["+"]*Ratio == Pr["F+"]*Pr["M+"]^-1)) 
...
p_inc = case_df_by_age_and_sex %>% 
  drop_na(incidence) %>% 
  ggplot(aes(y = incidence, x = as.factor(Age_group))) + geom_bar(
