...
create_model <- function(data, population, fits_coeff, location, prop_immune0, sir_gamma,
  ...
  params0 <- c(1.348612e-07*100, #prop i0
               ...
               rep(0.1,length(ts))) #weekly scaling
  ...
  simulator <- sir_model$new(
    ...
    interp_ts = ts_int,
    interp_scaling = interp_scaling(s_v = params0[5:length(params0)]),
  ...
  simulate <- function(params, t_points) {
    ...
    simulator$set_user(
      ...
      interp_scaling = interp_scaling(s_v = params[5:length(params0)])
    )
  ...
  observe <- function(sims, params) {
    ...
    rnbinom(nrow(sims), mu = params[3] * sims[, "i_inf"], size = exp(params[4])) #i_inf incialmente
  }
  ...
  compute_loglik <- function(params) { 
    ...
    expected <- params[3] * sims[, "i_inf"] #i_inf incialmente
  ...
  is_invalid <- function(k, value) {
    ...
    if ((k > 4) & ((value < 0) | (value>1))) return(TRUE) # Weekly scaling
    FALSE
  }
  ...
  # Indices to update
  ##############################################################################
  inds_to_update <- (1:length(params0))[-c(1,4)]
